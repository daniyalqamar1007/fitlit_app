...
TextEditingController messagecontroller=TextEditingController();
  ...
  String _detectSwipeRegion(Offset localPosition, Size imageSize) {
    // Convert local position to image coordinates
    double relativeY = localPosition.dy / imageSize.height;
    int pixelY = (relativeY * 1536).round();

    if (pixelY >= AvatarRegion.SHIRT_START && pixelY <= AvatarRegion.SHIRT_END) {
      return 'shirt';
    } else if (pixelY >= AvatarRegion.PANT_START && pixelY <= AvatarRegion.PANT_END) {
      return 'pant';
    } else if (pixelY >= AvatarRegion.SHOE_START && pixelY <= AvatarRegion.SHOE_END) {
      return 'shoes';
    }
    return 'shirt'; // Default to shirt if detection fails
  }
  void _handleAvatarStatusChange() {
    if (mounted) {
      setState(() {
  ...
  Future<void> _generateAvatar(BuildContext context) async {
    ...
    if (selectedShirtId == null ||
      ...
      showAppSnackBar(context, localizations.pleaseSelectAtLeastOneItem, backgroundColor: appcolor);
    ...
    try {
      ...
      showAppSnackBar(context, localizations.errorGeneratingAvatar(e.toString()), backgroundColor: appcolor);
